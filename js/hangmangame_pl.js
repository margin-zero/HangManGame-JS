...
function categoryListItemClick($this) {
    ...
    $(".information").text("Naciśnij przycisk 'Nowa Gra' aby rozpocząć nową zagadkę");
...
function letterClick($this) {
    ...
    if (errors_left>=0) {
        ...
        if (game_in_progress) {
            ...
            if (i==0) {  // i = 0 means no white space characters in question, so answer is completed
                $(".game-letters").hide();
                $(".information").text("Wygrana!").addClass("success");
                $(".information").show();
    ...
    if (errors_left<0) {
        $(".game-letters").hide();
        $(".information").text("Przegrana!... Odpowiedź to: " + current_questions[question_id-1]).addClass("failure");
        $(".information").show();
